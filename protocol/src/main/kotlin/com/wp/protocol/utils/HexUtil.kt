...
object HexUtil {
     ...
     */
    @JvmOverloads
    fun encodeHex(data: ByteArray, toLowerCase: Boolean = false): CharArray {
     ...
     */
    @JvmOverloads
    fun encodeHexStr(data: ByteArray?, toLowerCase: Boolean = false): String {
     ...
     */
    @JvmOverloads
    fun encodeHexStr(data: List<Byte>?, toLowerCase: Boolean = false): String {
        return encodeHexStr(data?.toByteArray())
    }
    ...
    fun encodeHexStr(data: ByteArray?, toDigits: CharArray): String {
        return if (data == null) {
            ""
        } else String(encodeHex(data, toDigits))
    }
    ...
    fun decodeHex(data: String?): ByteArray {
        return if (data == null) {
            ByteArray(0)
        } else decodeHex(data.toCharArray())
    }
    ...
    fun byteToInt(bb: ByteArray, index: Int, flag: Boolean): Int {
        return if (flag) {
            (bb[index + 0].toInt() and 0xff shl 24
                    or (bb[index + 1].toInt() and 0xff shl 16)
                    or (bb[index + 2].toInt() and 0xff shl 8)
                    or (bb[index + 3].toInt() and 0xff shl 0))
        } else {
            (bb[index + 3].toInt() and 0xff shl 24
                    or (bb[index + 2].toInt() and 0xff shl 16)
                    or (bb[index + 1].toInt() and 0xff shl 8)
                    or (bb[index + 0].toInt() and 0xff shl 0))
        }
    }
    ...
    fun byteToLong(data: ByteArray, index: Int, count: Int, flag: Boolean): Long {
        ...
        return if (flag) {
            when (count) {
                2 -> lg = (data[index + 0].toLong() and 0xffL shl 8
                        or (data[index + 1].toLong() and 0xffL shl 0))

                4 -> lg = (data[index + 0].toLong() and 0xffL shl 24
                        or (data[index + 1].toLong() and 0xffL shl 16)
                        or (data[index + 2].toLong() and 0xffL shl 8)
                        or (data[index + 3].toLong() and 0xffL shl 0))

                8 -> lg = (data[index + 0].toLong() and 0xffL shl 56
                        or (data[index + 1].toLong() and 0xffL shl 48)
                        or (data[index + 2].toLong() and 0xffL shl 40)
                        or (data[index + 3].toLong() and 0xffL shl 32)
                        or (data[index + 4].toLong() and 0xffL shl 24)
                        or (data[index + 5].toLong() and 0xffL shl 16)
                        or (data[index + 6].toLong() and 0xffL shl 8)
                        or (data[index + 7].toLong() and 0xffL shl 0))
            }
            lg
        } else {
            when (count) {
                2 -> lg = (data[index + 1].toLong() and 0xffL shl 8
                        or (data[index + 0].toLong() and 0xffL shl 0))

                4 -> lg = (data[index + 3].toLong() and 0xffL shl 24
                        or (data[index + 2].toLong() and 0xffL shl 16)
                        or (data[index + 1].toLong() and 0xffL shl 8)
                        or (data[index + 0].toLong() and 0xffL shl 0))

                8 -> lg = (data[index + 7].toLong() and 0xffL shl 56
     ...
     */
    @JvmOverloads
    fun bytesToHexString(b: ByteArray, length: Int = b.size): String {
    ...
    /**
     * 将byte list 转换为16进制数组成的字符串
     */
    @JvmOverloads
    fun bytesToHexString(b: List<Byte>, length: Int = b.size): String {
        return bytesToHexString(b.toByteArray())
    }
}